...
def get_stats(collections=['blacklisted', 'benign']):
    all_stats: Dict[str, Optional[Stats]] = dict.fromkeys(
        collections, None)  # type: ignore # will definitely be Stats or None
    ...
    for collection in collections:
        ...
        stats = Stats(
            ...
            total=count,
            missing={},
            txt_verification_present_count=0,
            txt_verifications={},
            rdap_last_change_median=0,
            most_common_tlds=[],
            most_common_registrars=[],
            most_common_countries=[],
            most_common_tls_authorities=[],
            average_ip_count=None,
            average_ns_count=None,
            average_tls_chain_length=None,
            average_tls_cert_validity=None,
            average_nerd_score=None,
            lex_averages={})
        rdap_last_changes = []
        tlds = []
        registrars = []
        ...
        for data in cursor:
                ...
                for txt in data['dns']['TXT']:
                    for verification in txt_verified_strings:
                        if verification in txt:
                            found_any = True
                            stats['txt_verifications'][verification] = stats['txt_verifications'].get(
                                verification, 0) + 1
                if found_any:
            ...
            if data['rdap'] and 'registrar' in data['rdap']['entities']:
                ...
                # type: ignore # filtered out entities without names
                names = map(lambda x: x['name'], entities_with_names)
